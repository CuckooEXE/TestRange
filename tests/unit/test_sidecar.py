...
class TestSidecarNicSpecs:
    ...
    def test_nat_adds_uplink_nic(self) -> None:
        sw = Switch(
            "sw", Network("a"), cidr="10.0.0.0/24", uplink="eth0", sidecar=Sidecar(nat=True)
        )
        specs = sidecar_nic_specs(sw)
    ...
    def test_static_uplink_addr_pins_eth1_ip(self) -> None:
        ...
        sw = Switch(
            ...
            cidr="10.0.0.0/24",
            uplink="eth0",
            sidecar=Sidecar(nat=True, addr=StaticAddr("10.10.10.2/24", gw="10.10.10.1")),
        )
...
class TestRenderSidecarInterfaces:
    def test_basic_static_eth0(self) -> None:
        sw = Switch("sw", Network("a"), cidr="172.31.0.0/24", sidecar=Sidecar(dhcp=True))
...
class TestRenderDnsmasqConf:
    def test_nat_only_is_quiescent(self) -> None:
        # A sidecar with neither dhcp nor dns (nat only) keeps dnsmasq quiet:
        # the DNS listener is off (port=0) and no DHCP range is served.
        sw = Switch(
            "sw", Network("a"), cidr="10.0.0.0/24", uplink="eth0", sidecar=Sidecar(nat=True)
        )
    ...
    def test_dhcp_plus_dns(self) -> None:
        sw = Switch("sw", Network("a"), cidr="10.0.0.0/24", sidecar=Sidecar(dhcp=True, dns=True))
        conf = render_dnsmasq_conf(sw, [], _mac_for)
    ...
    def test_static_uplink_dns_adds_upstream_forwarder(self) -> None:
        ...
        sw = Switch(
            ...
            sidecar=Sidecar(
                ...
                dns=True,
                nat=True,
                addr=StaticAddr("10.10.10.2/24", gw="10.10.10.1", dns=("1.1.1.1", "9.9.9.9")),
            ),
        )
        conf = render_dnsmasq_conf(sw, [], _mac_for)
...
class TestRenderNftablesRuleset:
    def test_no_nat_empty(self) -> None:
        # A non-NAT sidecar (dhcp/dns only) installs no nftables rules.
        sw = Switch("sw", Network("a"), sidecar=Sidecar(dhcp=True))
        rules = render_nftables_ruleset(sw)
        assert rules.strip() == "flush ruleset"

    def test_nat_emits_masquerade(self) -> None:
        sw = Switch("sw", Network("a"), uplink="eth0", sidecar=Sidecar(nat=True))
        rules = render_nftables_ruleset(sw)
...
class TestRenderSysctlConf:
    def test_no_nat_empty(self) -> None:
        assert render_sysctl_conf(Switch("sw", Network("a"), sidecar=Sidecar(dhcp=True))) == ""
...
def test_dhcp_pool_uses_addressing_consts_bounds() -> None:
    sw = Switch("sw", Network("a"), cidr="172.16.0.0/24", sidecar=Sidecar(dhcp=True))
    conf = render_dnsmasq_conf(sw, [], _mac_for)
