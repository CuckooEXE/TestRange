...
class TestInstallPhasePostInstallHook:
    """The :meth:`Builder.post_install_hook` re-boots the just-installed
    VM on the install network, runs an SSH-driven hook, then powers
    off again — all *between* install SHUTOFF and the cache snapshot.
    For builders whose ``has_post_install_hook()`` returns ``False``
    (cloud-init / Windows / NoOp), the re-boot dance is skipped so the
    cache-hit path stays unchanged."""

    def _patch_install_with_hook(
        self,
        vm: VM,
        ...
        *,
        ...
        dom.isActive.return_value = False
        import libvirt
        # Always SHUTOFF — both the post-install poll and the
        # post-hook poll should resolve immediately.
        dom.state.return_value = (libvirt.VIR_DOMAIN_SHUTOFF, 0)
        ...
        cache = MagicMock(spec=CacheManager)
        cache.store_vm.return_value = Path("/cache/web01.qcow2")
        ...
        install_spec = InstallDomain(
            work_disk="/tmp/web01-install.qcow2",
            seed_iso="/tmp/web01-seed.iso",
            extra_cdroms=(),
            uefi=False,
            windows=False,
            boot_cdrom=False,
        ...
        monkeypatch.setattr(
            vm.builder, "has_post_install_hook", lambda: has_hook,
        )
        monkeypatch.setattr(
            "testrange.backends.libvirt.vm._POLL_INTERVAL", 0,
        )
    ...
    def test_no_hook_skips_reboot(
        self, basic_vm: VM, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        # Default builder (no hook): one create() call, hook never
        # runs.  Cache snapshot happens after the single SHUTOFF.
        dom, conn, cache, run = self._patch_install_with_hook(
            basic_vm, monkeypatch, has_hook=False,
        )
        hook = MagicMock()
        monkeypatch.setattr(basic_vm.builder, "post_install_hook", hook)

        result = basic_vm._run_install_phase(
            conn=conn, cache=cache, run=run,
            install_network_name="tr-i",
            install_network_mac="52:54:00:00:00:01",
            install_network_ip="192.168.249.10",
            h="cafebabe",
        )
        assert result == Path("/cache/web01.qcow2")
        # Single create — install boot only.
        assert dom.create.call_count == 1
        hook.assert_not_called()

    def test_hook_runs_between_shutoff_and_snapshot(
        self, basic_vm: VM, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        ...
        dom, conn, cache, run = self._patch_install_with_hook(
            basic_vm, monkeypatch, has_hook=True,
        )

        hook = MagicMock()
        monkeypatch.setattr(basic_vm.builder, "post_install_hook", hook)

        # Capture the SSHCommunicator constructor args.
        ssh_instances: list[MagicMock] = []

        def _fake_ssh_factory(*args: object, **kwargs: object) -> MagicMock:
            comm = MagicMock()
            comm.exec.return_value = ExecResult(0, b"", b"")
            comm._kwargs = kwargs
            comm._args = args
            ssh_instances.append(comm)
            return comm

        monkeypatch.setattr(
            vm_mod, "SSHCommunicator", _fake_ssh_factory,
        )

        # Order tracking — assert hook runs between create() #2 and
        # store_vm.  Side-effect lambdas use semicolons (statements
        # not expressions) so mypy doesn't confuse list.append's None
        # return for a meaningful value.
        call_log: list[str] = []
        def _on_create(*_args: object) -> None:
            call_log.append("create")
        def _on_shutdown(*_args: object) -> None:
            call_log.append("shutdown")
        dom.create.side_effect = _on_create
        dom.shutdown.side_effect = _on_shutdown
        original_hook = hook

        def hook_wrapper(
            _vm: object, comm: object,
        ) -> object:
            call_log.append("hook")
            return original_hook(_vm, comm)
        monkeypatch.setattr(basic_vm.builder, "post_install_hook", hook_wrapper)

        def _on_store_vm(*_args: object, **_kwargs: object) -> Path:
            call_log.append("store_vm")
            return Path("/cache/web01.qcow2")
        cache.store_vm.side_effect = _on_store_vm
        ...
        # Two create()s: install boot + post-install reboot for hook.
        assert dom.create.call_count == 2
        # Hook called once with the SSHCommunicator instance.
        original_hook.assert_called_once()
        comm_arg = original_hook.call_args.args[1]
        assert comm_arg is ssh_instances[0]
        # SSHCommunicator targeted the install IP with root cred.
        assert comm_arg._kwargs.get("host") == "192.168.249.10"
        assert comm_arg._kwargs.get("username") == "root"
        assert comm_arg._kwargs.get("password") == "pw"

        # Order: install create → hook reboot create → hook → shutdown → store.
        # State polls between events are filtered out.
        assert call_log == [
            "create",     # install boot
            "create",     # hook reboot
            "hook",
    ...
    def test_hook_failure_destroys_domain_and_propagates(
        self, basic_vm: VM, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        ...
        # The hook reboot leaves the domain active until our shutdown.
        # Set isActive=True so the finally block destroys it.
        dom.isActive.return_value = True

        monkeypatch.setattr(
            vm_mod, "SSHCommunicator", lambda **kw: MagicMock(),
        )
        monkeypatch.setattr(
            basic_vm.builder, "post_install_hook",
            MagicMock(side_effect=CloudInitError("bootstrap exited 100")),
        )
        ...
        # Cleanup still ran — finally block is unconditional.
        dom.destroy.assert_called()
        dom.undefineFlags.assert_called_once()
        # Cache must NOT have been written (broken artifact).
        cache.store_vm.assert_not_called()
        assert basic_vm._install_domain is None
