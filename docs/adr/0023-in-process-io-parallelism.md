# ADR-0023: Bounded in-process parallelism in the orchestration I/O phases
...
1. **One shared connection, driven concurrently.** All workers use
   `ctx.driver`. Per-worker connections were rejected (see Context): a single
   instance is what keeps the in-memory resource maps usable at all. But "one
   instance" is *not* on its own thread-safety — those maps
   (`_libvirt_net_by_network`, `_vnet_by_network`) and the drivers' lazy-init
   slots (libvirt's serial dir/listener map, proxmox's SSH client) are written
   from worker threads, so each driver guards them with a per-driver
   `_state_lock` (and the clients' `call_lock` for the lazy connects). On
...
- A single `parallel_map` helper owns the worker cap (`--jobs`, default
  `DEFAULT_MAX_WORKERS=8`; `0` or `1` forces serial) and the fail-fast policy:
  the first worker exception propagates (with its traceback). Workers not yet
  started are cancelled; workers already in flight are awaited — the executor
  joins them on exit — so a sibling that already created a backend resource
  finishes recording it before teardown runs (no abandoned mid-RMW worker, no
  leak). The propagated exception is the earliest *submitted* failure among
  those that have *completed*; exact which-worker-won determinism is not
  promised when several are in flight.
...
- **Teardown of the build infra** — on the success path `teardown_build_phase`
  destroys in fixed order (sidecar → networks → switch → pool). On a *failure*
  mid-build the exception propagates to the orchestrator, which runs the
  ledger-driven `teardown` instead: it reverses whatever was recorded in
  `state.json`. Because the parallel build interleaves `record_intent` calls,
  that ledger order is scheduling-dependent — but build-infra destroys are
  order-tolerant (the pool is recorded first and destroyed last; VMs/volumes
  before their pool), so reversal still reclaims everything. The invariant is
  "everything created is reachable for teardown," not a byte-exact LIFO order
  under failure.
...
- The thread-safety substrate (StateStore lock, `ctx.ledger_lock`, the per-driver
  `call_lock` for agent/lazy-init ops and `_state_lock` for the resource maps +
  Proxmox SDN, the cache `_write_lock` + `mkstemp` + alias-merge fix) is a
  permanent invariant: any new shared mutable state touched inside a phase must
  be guarded the same way, and any new driver must keep its connection — *and
  its own in-memory maps* — usable from several threads.
