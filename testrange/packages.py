...
class Pip(AbstractPackage):
    ...
    def __init__(self, name: str, user_install: bool = False) -> None:
        super().__init__(name)
    ...
    def install_commands(self) -> list[str]:
        ...
        """
        flag = " --user" if self.user_install else ""
        return [f"pip3 install{flag} {self.name}"]

    def __repr__(self) -> str:
        if self.user_install:
            return f"Pip({self.name!r}, user_install=True)"
