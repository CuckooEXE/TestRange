...
# concurrent control-plane calls on the one shared session (ADR-0020), so a high
...
class ProxmoxClient:
    ...
    def __init__(self, conn: ProxmoxConn) -> None:
        ...
        # I/O phases drive it from several worker threads (ADR-0020): QGA agent
    ...
    def _widen_session_pool(self) -> None:
        """Resize the proxmoxer session's connection pool for ADR-0020 concurrency.
    ...
    def _ensure_ssh(self) -> Any:
        # Double-checked under call_lock: a parallel build (ADR-0020) runs
